...
@Component({
  ...
  styleUrls: ['./it-audit.component.scss']
})
export class ItAuditComponent implements OnInit {
  ...
  setPage() {
    ...
    this.auditService.getAllPageableForItAudit(this.page).subscribe(pagedData => {
  ...
  pass(eventId) {
    this.auditService.pass(eventId, null, this.currentUser.id).subscribe(res => {
      if (res) {
